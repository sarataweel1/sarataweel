...
$commentQuery = "SELECT comment-id, comment-parent-id, comment, comment-sender, date FROM comment-live WHERE comment-parent_id = '0' ORDER BY id DESC";
...
function getCommentReply($conn, $parentId = 0, $marginLeft = 0) {
	$commentHTML = '';
	$commentQuery = "SELECT comment-id, comment-parent-id, comment, comment-sender, date FROM comment-live WHERE comment-parent_id = '".$parentId."'";	
